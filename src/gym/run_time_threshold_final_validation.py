...
VARIANTS = [
    ...
    {
        "name": "buffer_time_threshold_best_step",
        "flags": {
...
def write_report(output_dir, summary):
    ...
    best_step = summary["variants"]["buffer_time_threshold_best_step"]
    ...
    for name in ["baseline", "buffer_default_step", "buffer_time_threshold_best_step"]:
        variant = summary["variants"][name]
        lines.append(
            "| %s | `%0.3f +/- %0.3f` | `%0.3f +/- %0.3f` | `%0.6f +/- %0.6f` | `%0.6f +/- %0.6f` | `%0.6f +/- %0.6f` | `%0.6f +/- %0.6f` |"
            % (
                name,
    ...
    for index, seed in enumerate(seeds):
        seed_values = {
            ...
            "buffer_time_threshold_best_step": best_step["runs"][index]["metrics"]["mean_avg_peak_buffer_utilization"],
        }
        best_name = min(seed_values, key=seed_values.get)
        if best_name == "buffer_time_threshold_best_step":
            win_count += 1
        lines.append(
            "| %d | `%0.6f` | `%0.6f` | `%0.6f` | `%s` |"
            % (
                ...
                seed_values["buffer_time_threshold_best_step"],
                best_name,
            )
        )
    ...
    candidate_peak = best_step["mean_avg_peak_buffer_utilization_mean"]
    ...
    if candidate_peak < baseline_peak:
        verdict = "The final candidate beats baseline on the primary peak-buffer metric."
    elif candidate_peak < default_peak:
        verdict = "The final candidate beats `buffer_default_step`, but still does not beat baseline."
    else:
        verdict = "The final candidate does not beat either baseline or `buffer_default_step` on the primary peak-buffer metric."

    lines.extend([
        verdict,
        "",
        "- vs baseline avg peak buffer: `%+.2f%%`" % percent_change(candidate_peak, baseline_peak),
        "- vs baseline avg buffer: `%+.2f%%`" % percent_change(
            best_step["mean_avg_buffer_utilization_mean"],
            baseline["mean_avg_buffer_utilization_mean"],
        ),
        "- vs baseline time above threshold: `%+.2f%%`" % percent_change(
            best_step["mean_avg_buffer_time_above_threshold_fraction_mean"],
            baseline["mean_avg_buffer_time_above_threshold_fraction_mean"],
        ),
        "- vs baseline return: `%+.2f%%`" % percent_change(
            best_step["mean_episode_return_mean"],
            baseline["mean_episode_return_mean"],
        ),
        "- vs `buffer_default_step` avg peak buffer: `%+.2f%%`" % percent_change(candidate_peak, default_peak),
        "- candidate wins on avg peak buffer in `%d / %d` seeds" % (win_count, len(seeds)),
        "",
        "## Artifacts",
        "",
...
def main():
    ...
    output_dir = arg_or_default(
        "--output-dir",
        default=os.path.join(repo_root, "experiments", "time_threshold_final_validation_20260429"),
    )
    ...
    for seed in seeds:
        ...
        for variant in VARIANTS:
            ...
            print(
                "[done] %s avg_peak=%0.6f avg_buffer=%0.6f return=%0.3f"
                % (
                    run_name,
    ...
    print("Finished final time-threshold validation. Summary: %s" % summary_path, flush=True)
